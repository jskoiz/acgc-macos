...
branch, producing `80e80dfebe6c78ee5ad99eb418cf79b874e70e1e`. The decomp oracle
was `upstream/ac-decomp` `09ca8e8b`.
...
The integrated commit changes exactly five PC files:
...
cmake -S upstream/ACGC-PC-Port/pc \
  -B /private/tmp/acgc-integrated-texture-source-80e80df-native \
  -G 'Unix Makefiles' -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON \
  -DPC_DARWIN_COMPILE_AUDIT=ON
cmake --build /private/tmp/acgc-integrated-texture-source-80e80df-native \
  --target acgc_pc_gx_texture_source_record_fixture --parallel 1
ctest --test-dir /private/tmp/acgc-integrated-texture-source-80e80df-native \
  --output-on-failure --parallel 1 \
  -R '^acgc_pc_gx_texture_source_record_fixture$'
```

Result: build succeeded and `1/1` focused fixture passed. The expected legacy
...
```text
cmake -S upstream/ACGC-PC-Port/pc \
  -B /private/tmp/acgc-integrated-texture-source-80e80df-asan \
  -G 'Unix Makefiles' -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=ON \
  -DPC_DARWIN_COMPILE_AUDIT=ON \
...
cmake --build /private/tmp/acgc-integrated-texture-source-80e80df-asan \
  --target acgc_pc_gx_texture_source_record_fixture --parallel 1
ASAN_OPTIONS='detect_leaks=0:halt_on_error=1:abort_on_error=1' \
UBSAN_OPTIONS='halt_on_error=1:print_stacktrace=1' \
ctest --test-dir /private/tmp/acgc-integrated-texture-source-80e80df-asan \
  --output-on-failure --parallel 1 \
  -R '^acgc_pc_gx_texture_source_record_fixture$'
```

Result: build succeeded and `1/1` focused fixture passed with no sanitizer
...
The existing `acgc_pc_gx_semantic_v2_handoff_tests` was also rebuilt on the
integrated tip and on its exact parent `3c08c7f71`; both fail at the pre-existing
fixture expectation `v2_probe.status == ACGC_METAL_PACKET_CONSUMER_OK` at
`pc/tests/pc_gx_semantic_v2_handoff.c:303`. This lane did not change the
consumer and does not claim to fix or reinterpret that separate baseline.
