...
| 382 | Stale task/worktree cleanup | Complete, continued | Archived all fourteen prior visible worker tasks and the superseded pinned predecessor; the current orchestrator is the only unarchived visible ACGC task in the latest app listing. Cumulative verified cleanup removed 75 clean/integrated registered worktrees plus one broken decomp registration, retired two detached audit clones, and moved 365 audited generated/temporary directory roots recoverably to macOS Trash. This continuation removed three of those clean worktrees, separately quarantined 17 stale PR-body/list/dry-run files (64,369 bytes), and moved twelve owner-confirmed superseded harness/cache/preflight roots (8.5 MiB) to `/Users/jk/.Trash/acgc-cleanup-20260824-trace-download.5t7QY2`. Current candidates, exact-merge proof roots, populated oracles, the retained exact-`9860` and exact-`586` traces/builds, dirty/unique objects, registered source worktrees, and asset-linked roots remain preserved; no disc material was moved or deleted. |
...
| 389 | Supply-chain and verification audit | Held; P1 corrections required | The exact-current replay at candidate tip `8b9f53ba2` passes `37/37` offline tests in normal and `PYTHONMALLOC=debug` modes, but independent review found three blockers: backup-root path substitution can redirect recursive cleanup, temporary-entry substitution can destroy a pre-existing destination without rollback, and the unconditional POSIX dirfd/fcntl gate rejects every Windows download/header operation. The manifest also needs a machine-auditable acquisition-snapshot/integration-base policy. No corrective commit, push, network operation, payload execution, Windows runtime, or asset access followed; one read-only Windows handle-relative design lane is active before any source owner is appointed. |
...
| 410 | Active Texture/TEV/Dynamic resource contract | Complete and integrated | The converged audits appointed one owner and prohibited Texture-only admission. The five-commit borrow-scoped CPU resource transport and atomic plan/resource callback ownership chain passed independent candidate review, merged through [PC PR #35](https://github.com/jskoiz/ACGC-PC-Port/pull/35) as `586cf7a61`, and passed fresh exact-merge PC `3/3` plus Apple `3/3` native and combined ASan/UBSan matrices. The later exact-merge trace proves live resource callback/staging success but retains status 17 and no sink; no Metal or pixel success is claimed. |
| 411 | Projection reconstruction current-tip audit | Complete; successor integrated | Exact-`9860ebc5c` raw capture, canonical producer, plan decode, and consumer reconstruction match `GXSetProjection`/`GXSetProjectionv`; the retained trace validates the producer/plan path but stops at Texture before consumer transform construction. The sole non-critical gap was nonidentity orthographic test coverage; Lane 417 closes it without production or CMake changes. |
...
20. ~~Determine whether the current eight-map Texture state, two-stage TEV use,
   ...
   fresh exact-merge PC/Apple native plus combined ASan/UBSan proof. One exact
