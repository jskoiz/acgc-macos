...
dependency-ready work. The current canonical PC tip is `07929bf6a`; see
...
| 382 | Stale task/worktree cleanup | Complete, continued | Archived all fourteen prior visible worker tasks and later archived the superseded pinned predecessor integration task. The registered passes removed 65 clean/integrated worktrees plus one broken decomp registration; later conservative passes retired four more clean superseded registered worktrees, two detached audit clones, and brought the explicitly audited generated or temporary roots moved to macOS Trash to 38. Current candidates, exact-merge proof roots, populated oracles, dirty/unique objects, and roots containing a local disc link remain preserved. Git-backed removals are reproducible; temporary-root removals are recoverable from Trash. |
...
| 405 | Nested ROM ignore policy | Exact-current replay active | Read-only replay audit found local candidate `f690adbf8` unique and conflict-free at `9860ebc5c`: PR #27 covers only direct runtime roots, while `pc/build32/bin/rom/YourGame.ciso` remains outside those anchored rules. One exact-current hygiene owner is replaying only recursive mixed-case `.iso`/`.gcm`/`.ciso` policy plus a dummy-path failure-propagation fixture; no image may be read, moved, deleted, hashed, or published. |
...
| 412 | J2D plan umbrella integration | Integration candidate; immutable review pending | This five-path integration advances the PC gitlink and canonical runner pin to `07929bf6a`, synchronizes the plan-only claim, and records exact candidate plus exact-merge native/ASan proof. No full link, process, producer, consumer, Metal, pixel, device, or playability claim follows. |
...
21. Replay the reviewed download-manifest and nested-ROM hygiene changes one at
   ...
   nested-ROM exact-current replay is active and the download replay remains
   queued.
