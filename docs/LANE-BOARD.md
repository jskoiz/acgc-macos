...
playability claim follows. Lane 101 is now the sole active worker, with a
separate source-edit contract for the smallest versioned extension that can
represent this observed state.
Lane 94
...
| 101 | Live blend/texture-matrix GX packet extension — `019ffd19-3a91-7ba2-b6db-c7535d5143ce` | Source-edit lane for the smallest versioned packet/Apple consumer extension covering the observed `GX_BM_BLEND` + `GX_TEXMTX0` state; preserve v1 and legacy OpenGL | Visible task worktree `/Users/jk/.codex/worktrees/fb3c/acgc-modern-port`; owning PC branch/worktree and focused roots to be declared before edits; base umbrella `6c03d29`, PC `8a19f23`, decomp `09ca8e8b` | Active/setup complete; no source edit, build, full link, LLDB, callback, Metal, pixel, or playability result yet |
