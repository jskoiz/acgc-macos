...
`orig/` are present. Lane 141 is the sole active root-owned runtime gate. Full
links and LLDB remain serialized, and remote workers may not update the
umbrella checkout.
...
focused triangle-batch source gate. Lane 141 owns the only serialized current-
tip full link/LLDB trace; no competing runtime, duplicate, or filler lane is
open. The current portable
...
| 141 | Current-tip V2 triangle-batch runtime — root-owned | Exactly one serialized native arm64 `ac_pc` link and one bounded logged-in GUI-session LLDB launch at PC `c973dbee`; count direct/batch V2 builders, typed Apple consumer/prepare/provider/runtime observer, graph/GX, and source-line rejection frontiers; no source edit | Build `/private/tmp/acgc-current-v2-triangle-runtime-c973dbee`; trace `/private/tmp/acgc-current-v2-triangle-runtime-c973dbee-logs`; canonical PC `c973dbee`; decomp `09ca8e8b`; local ignored ISO symlink only | Active/setup; no competing full link/LLDB; require exact link/hash, PIDs, boot/LOGO/NEOS, per-symbol/return-line counts, TERM/KILL disposition, and fail-closed next blocker; no successful Metal encode/present/readback, pixel, input/audio/save/device/natural-shutdown/playability inference |
