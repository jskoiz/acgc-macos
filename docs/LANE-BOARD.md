...
| 122 | CARD production validation — `019fff17-0fe1-7b23-8dd9-b18503b70fe8` | Remote M3 Max source lane; own only `pc/src/pc_m_card.c`, one production validation fixture, and minimal registration; no GX/Apple edits | Worktree `/private/tmp/acgc-lane-card-production-validation-m3`; branch `c1/lane-card-production-validation-m3`; base `a53b192` | Active/two-upstream wire-layout crosswalk; no physical CARD/device/persistence/playability claim |
| 123 | Lifecycle timing audit — `019fff17-1964-7822-babf-2120bc78fb6e` | Remote M3 Max read-only/test-only timing, retrace, supervisor TERM grace, KILL fallback, and shutdown ownership audit | Roots `/private/tmp/acgc-lane-lifecycle-timing-m3-native` and `-asan`; base `a53b192` | Active; synthetic lifecycle evidence only, no normal game teardown/playability claim |
| 124 | Graph terminator fixture — `019fff17-22e0-7f12-8c20-2fad9a684892` | Remote M3 Max test-only fixture for indirect target resolution and exact `DF000000,0` termination; own only focused test/CMake | Worktree `/private/tmp/acgc-lane-graph-terminator-fixture-m3`; branch `c1/lane-graph-terminator-fixture-m3`; base `a53b192` | Active; synthetic COMPLETE/PREFIX_ONLY/UNTERMINATED/MALFORMED tests pass natively; ASan/UBSan pending; no live graph/draw/frame claim |
| 125 | Texture/TLUT/TEV fixtures — `019fff17-2f40-78c1-b2c6-f69c50fc93fb` | Remote M3 Max test-only fixture lane; own only texture/TEV fixture files and minimal CMake; no packet-header, `pc_gx.c`, or Apple edits | Worktree `/private/tmp/acgc-lane-texture-tev-fixtures-m3`; branch `c1/lane-texture-tev-fixtures-m3`; base `a53b192` | Active/crosswalk; no live callback, Metal, frame, or pixel claim |
| 126 | Metal state contract audit — `019fff17-38e4-7ed3-a2aa-04e48b823c33` | Remote M3 Max read-only CPU/contract audit of the Metal sink/state encoder seam and GX state mapping; no source/build/link/launch edits | Log root `/private/tmp/acgc-lane-metal-state-contract-m3`; base `a53b192` | Active; identifies the next state/encoder gate only; no device/encode/readback/pixel/playability claim |
| 127 | iOS shared-boundary audit — `019fff17-4100-78f1-91c4-0996c40e41b5` | Remote M3 Max read-only portable/Apple boundary readiness audit; no iOS target, simulator/device, or source edits | Root `/private/tmp/acgc-lane-ios-boundary-m3`; base `a53b192` | Active; portable/shared-boundary inventory only; no iOS playability claim |
