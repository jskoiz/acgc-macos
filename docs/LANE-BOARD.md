...
are complete/integrated/archived. Lane 111 remains the sole serialized
read-only runtime worker, while lanes 112–114 are now active independent CPU
lanes: production Save_t/CARD recovery, input-boundary verification, and
mixer/CoreAudio audit. No lane may start a competing full link or LLDB trace;
no duplicate or filler lane is open. The current portable verification tip is
`dbf6986`, which
...
untouched. The authoritative PC source is `dbf6986` on
...
focused CTest `6/6` each. Lane 111 is the serialized read-only runtime lane;
lanes 112–114 are the active non-runtime CPU/read-only lanes. Full links and
LLDB launches remain serialized.
...
| 112 | Production Save_t/CARD recovery — `019ffdba-e4f1-71d1-82fd-57561a66e50a` | Source-edit lane for the two-upstream Save_t/GCI checksum and main/backup recovery seam; own only `pc/src/pc_m_card.c`, one focused recovery fixture, and minimal registration; no full link/LLDB/device | Visible worktree `/Users/jk/.codex/worktrees/6e5b/acgc-modern-port`; canonical PC base `c1/macos-host-launch` `dbf6986`; decomp `09ca8e8b`; branch `c1/lane-card-production-recovery` to be created before edits; unique roots `/private/tmp/acgc-lane-card-production-recovery-*` (not created yet) | Active/setup; crosswalk finds the production validation/fallback shape already present and is narrowing the smallest justified test-registration/source delta; native plus combined ASan/UBSan focused tests only (`--parallel 1`, `detect_leaks=0`); no physical CARD/device/persistence/playability claim |
| 113 | Input snapshot boundary audit — `019ffdba-e4ee-72c3-ad9a-5f9d77153f34` | Read-only/test-only characterization of per-frame `PCInputSnapshot`, controller/keyboard mapping, and game-owned frame guard; no source/CMake edits, full link, LLDB, physical input, or playability | Visible worktree `/Users/jk/.codex/worktrees/8a82/acgc-modern-port`; canonical PC `c1/macos-host-launch` `dbf6986`; decomp `09ca8e8b`; unique roots `/private/tmp/acgc-lane-input-runtime-boundary-*` (one probe root created under this prefix) | Active/verification; native and combined ASan/UBSan focused input/frame-guard tests are `3/3` each with no diagnostics; synthetic SDL keyboard state remains OS/human-only, and a double-`PADRead` stability probe is pending; no physical-input/simulator/device/playability claim |
| 114 | Mixer/DMA/CoreAudio boundary audit — `019ffdba-e4f1-71d1-82fd-573f767a436b` | Read-only verification of the JAudio mixer/DMA/NEOS-to-Apple sink boundary; no source edits, full link, LLDB, ISO/assets, or audible-device claim | Visible worktree `/Users/jk/.codex/worktrees/0705/acgc-modern-port`; canonical PC `c1/macos-host-launch` `dbf6986`; decomp `09ca8e8b`; unique roots `/private/tmp/acgc-lane-mixer-coreaudio-current-*` | Active/verification; focused CMake-scoped audio targets are built in native and ASan/UBSan roots and tests are running; record exact CoreAudio/device skip and diagnostics; no audible audio or playability claim |
