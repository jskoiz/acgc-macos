...
dependency-ready work. The current canonical PC tip is `d40ca1c2c`; see
`docs/evidence/TYPED-GEOMETRY-CONSUMER-D40CA1C2C-2026-08-23.md`.
...
| 399 | Logical RGBA8 fixture correction | Reviewed replay ready | The verification matrix exposed two stale Apple plan expectations that decoded already-canonical logical RGBA8 words a second time. Independently reviewed patch `8921f4c2d` was replayed unchanged onto exact `d40ca1c2c` as `b716a46db`; its stable patch ID matches, and fresh serialized Apple plan plus root round-trip fixtures pass `1/1` natively and under combined ASan/UBSan. PC PR/merge and exact-merge gates remain pending. |
