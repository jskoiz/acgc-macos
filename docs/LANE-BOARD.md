...
dependency-ready work. The current canonical PC tip is `503194ff2`; see
`docs/evidence/POST-BASELINE-LIVE-TRACE-DISC-HYGIENE-503194FF2-2026-08-23.md`.
...
| 389 | Supply-chain and verification audit | Complete; implementation dependencies ready | Found P1 mutable/unhashed binary and `ultralib/main` downloads, plus fragmented executable/compile-only probes and an assert-based wave-reader registration hazard under global `NDEBUG`. Public-artifact digest/member acquisition is complete, and the deterministic verification-runner candidate is in immutable review; neither touches the Apple Geometry fix. |
...
| 392 | Apple Geometry-plan source owner | Active, single owner | Owns only `pc/apple/src/apple_canonical_plan.c` and the existing source-backed root round-trip fixture from exact PC `503194ff2`; it must consume canonical Geometry words once, add integer/packed-format regression coverage, and pass fresh native plus combined ASan/UBSan focused gates. Duplicate Texgen/Geometry investigations are parked. |
| 393 | Canonical verification runner | Candidate complete; immutable review active | Candidate `8c341052d` adds only one umbrella script and passes fresh root `1/1` plus Apple `4/4` matrices natively and under combined ASan/UBSan, with exact discovery, zero skips, and diagnostic scans. It does not touch CMake, submodules, production code, docs, workflows, or assets. |
