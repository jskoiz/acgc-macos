...
dependency-ready work. The current canonical PC tip is `621a4d548`; see
`docs/evidence/LOGICAL-RGBA8-FIXTURE-621A4D548-2026-08-23.md`.
...
| 382 | Stale task/worktree cleanup | Complete | Archived all fourteen prior visible worker tasks; the initial pass removed 62 registered clean/integrated worktrees, four empty directories, two zero-byte sentinels, and one broken decomp registration. A later conservative pass removed three more clean/superseded worktrees, three empty generated directories, and two zero-byte artifacts while preserving every current candidate, exact-merge proof root, populated oracle, dirty/unique object, and verification-runner worktree. Clean removals remain recoverable from Git objects or patch-equivalent refs. |
...
| 389 | Supply-chain and verification audit | Candidate reviewed; replay queued | Found P1 mutable/unhashed binary and `ultralib/main` downloads. Candidate `9e295b460` adds a 28-artifact manifest, pinned ultralib revision, size/SHA validation, safe staged archives, and verified offline cache handling; an independent review found no P0/P1/P2, 13/13 tooling tests pass, and the focused root round trip passes natively and under ASan/UBSan. It remains based on `6c5a626d9` and must be replayed and rerun on the current PC tip before any PR. |
...
| 393 | Canonical verification runner | Corrected candidate; integration blocked by exposed fixture | Corrected executable candidate `fc5ade965` adds complete dry-run/root guards and the full root/Apple native plus combined-sanitizer matrix. Its exact-`6c5a626d9` run correctly exposed stale Apple logical-RGBA8 expectations: root `1/1` passed while Apple passed `3/4` in native and sanitizer trees. The runner is not integrated; replay, a current PC pin, the repaired full matrix, and independent review remain required. |
...
| 400 | Active Texgen consumer predicate audit | Complete; sole future owner frozen | All production Texgen builders/validators and Apple plan construction already pass. The only future production owner is `pc/apple/src/metal_packet_consumer.c`; no duplicate Texgen edit may proceed. Source-backed evidence predicts one regular TEX0 generator with ordinary identity 60 and post identity 125, but the live record/matrix/SU/Texture/TEV fields were not captured, so one exact-`621a4d548` bounded field trace must precede the source edit. |
