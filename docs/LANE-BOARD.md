...
dependency-ready work. The current canonical PC tip is `6c5a626d9`; see
`docs/evidence/APPLE-GEOMETRY-CANONICAL-WORDS-6C5A626D9-2026-08-23.md`.
...
| 389 | Supply-chain and verification audit | Complete; implementation active | Found P1 mutable/unhashed binary and `ultralib/main` downloads, plus fragmented executable/compile-only probes and an assert-based wave-reader registration hazard under global `NDEBUG`. Public-artifact digest/member acquisition is complete; a disjoint tooling source owner is implementing the fail-closed manifest/cache/archive contract from exact PC `503194ff2`. |
...
| 393 | Canonical verification runner | Corrective replay active | Immutable review blocked the first candidate because its exact-da96 guard would reject the new `503194ff2` gitlink and noted missing `tee`/required-log failure checks. One new one-path candidate is being rebuilt from umbrella `4cea0363a`, hardened, and rerun against exact PC `503194ff2`. |
...
| 396 | Post-Geometry bounded live trace | Queued, serialized | After umbrella integration, build exact `6c5a626d9` once and run one symbol-name 60-second/20-attempt trace with exact-PID cleanup, stopping at the next Apple plan, typed consumer, or sink result. No Metal/pixel claim is implied. |
