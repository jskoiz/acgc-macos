...
dependency-ready work. The current canonical PC tip is `d472c6bd3`; see
`docs/evidence/NESTED-ROM-IGNORE-D472C6BD3-2026-08-24.md`.
...
| 382 | Stale task/worktree cleanup | Complete, continued | Archived all fourteen prior visible worker tasks and later archived the superseded pinned predecessor integration task. The registered passes removed 65 clean/integrated worktrees plus one broken decomp registration; later conservative passes retired seven more clean superseded registered worktrees, two detached audit clones, and brought the explicitly audited generated or temporary roots moved to macOS Trash to 124. Current candidates, exact-merge proof roots, populated oracles, dirty/unique objects, and roots containing a local disc link remain preserved. Git-backed removals are reproducible; temporary-root removals are recoverable from Trash. |
...
| 410 | Active Texture/TEV/Dynamic resource contract | Audits complete; one atomic source owner active | Both read-only audits prohibit Texture-only admission: the value plan has no lease/resource bytes, the V2 provider cannot prove canonical resource identity, and the current sink is geometry/color-only. Exactly one source owner now owns the non-V2 synchronous resource transport and fail-closed Texture+TEV+Indirect+Dynamic staging contract; duplicate production edits remain stopped. No renderer success is pre-claimed. |
...
| 414 | Nested ROM umbrella integration | Integration candidate | This five-path integration advances the PC gitlink and runner pin to `d472c6bd3`, synchronizes README/lane-board state, and records the policy/Python-only proof boundary. The runtime/full-link frontier remains exact `9860ebc5c`; no renderer or playability claim follows. |
...
20. ~~Determine whether the current eight-map Texture state, two-stage TEV use,
   ...
   the non-V2 synchronous resource-handoff owner is active and must remain
   fail-closed until Texture, TEV, Indirect, Dynamic, and resource staging are
...
21. Replay the reviewed download-manifest and nested-ROM hygiene changes one at
   ...
   download-integrity replay is under focused verification/review.
