...
dependency-ready work. The current canonical PC tip is `586cf7a61`; see
`docs/evidence/TEXTURE-RESOURCE-HANDOFF-586CF7A6-2026-08-24.md`.
...
| 382 | Stale task/worktree cleanup | Complete, continued | Archived all fourteen prior visible worker tasks and the superseded pinned predecessor; the current orchestrator is the only unarchived visible ACGC task in the latest app listing. Cumulative verified cleanup removed 75 clean/integrated registered worktrees plus one broken decomp registration, retired two detached audit clones, and moved 353 audited generated/temporary directory roots recoverably to macOS Trash. This continuation removed three of those clean worktrees and separately quarantined 17 stale PR-body/list/dry-run files (64,369 bytes). Current candidates, exact-merge proof roots, populated oracles, the retained `9860` trace-import chain/runtime dependency, dirty/unique objects, and asset-linked roots remain preserved; no disc material was moved or deleted. |
...
| 389 | Supply-chain and verification audit | Exact-current corrective candidate under immutable review | The four-path manifest/downloader chain was replayed from exact PC `d472c6bd3`; corrective tip `7a299cd32` descriptor-anchors cache, executable, archive-source, archive-output, and generated-header publication, rejects unsafe dirfd platforms before network or mutation, and passes `32/32` offline tests plus `py_compile`. It owns no C/Apple/CMake/runtime file and remains unpushed pending independent review against current PC `586cf7a61`. No network payload, downloaded-tool execution, Windows runtime, or asset claim follows. |
...
| 405 | Nested ROM ignore policy | Complete and integrated | Exact-current candidate `6e52b8f85` adds three recursive lowercase-`rom` mixed-case `.iso`/`.gcm`/`.ciso` rules plus a 123-line dummy-path fixture. Independent candidate and exact-merge review passed; [PC PR #34](https://github.com/jskoiz/ACGC-PC-Port/pull/34) merged it as `d472c6bd3`, and the exact merge passes `5/5` with zero tracked disc images. No image was read, moved, deleted, hashed, or published. |
...
| 408 | Texgen umbrella integration | Complete and integrated | Independent immutable review passed with no P0/P1; [umbrella PR #29](https://github.com/jskoiz/acgc-macos/pull/29) merged as `c10527334`, advancing the PC gitlink and canonical runner pin to `9860ebc5c` with synchronized README, lane board, and exact merge/link/trace evidence. The only hosted check was GitGuardian; no Apple workflow ran. |
| 409 | J2D four-vertex plan fixture | Complete and integrated | Candidate `b85cd5927` changes only `pc/apple/tests/test_apple_canonical_plan.c`, adds a source-shaped direct POS+CLR0 `GX_QUADS` plan and a TRIANGLES negative control, and passed independent immutable review. [PC PR #33](https://github.com/jskoiz/ACGC-PC-Port/pull/33) merged it as `07929bf6a`; fresh exact-merge native and combined ASan/UBSan fixtures each discover and pass `1/1`. It owns no production/CMake/runtime file. |
| 410 | Active Texture/TEV/Dynamic resource contract | Complete and integrated | The converged audits appointed one owner and prohibited Texture-only admission. The five-commit borrow-scoped CPU resource transport and atomic plan/resource callback ownership chain passed independent candidate review, merged through [PC PR #35](https://github.com/jskoiz/ACGC-PC-Port/pull/35) as `586cf7a61`, and passed fresh exact-merge PC `3/3` plus Apple `3/3` native and combined ASan/UBSan matrices. Status 17/no-sink policy remains unchanged; no full-link, live resource callback, Metal, or pixel success is claimed. |
| 411 | Projection reconstruction current-tip audit | Complete; no production defect | Exact-`9860ebc5c` raw capture, canonical producer, plan decode, and consumer reconstruction match `GXSetProjection`/`GXSetProjectionv`; the retained trace validates the producer/plan path but stops at Texture before consumer transform construction. PC `07929bf6a` changes only the plan fixture, so this production conclusion remains current. Only a non-critical test-only plan-decoder coverage successor remains. |
| 412 | J2D plan umbrella integration | Complete and integrated | Independent immutable review passed with no P0/P1; [umbrella PR #30](https://github.com/jskoiz/acgc-macos/pull/30) merged as `1a8f666e1`, advancing the PC gitlink and canonical runner to `07929bf6a`. Fresh runner proof was PC `1/1` plus Apple `4/4` natively and under combined ASan/UBSan; only GitGuardian ran remotely. |
| 413 | Nested ROM exact-merge review | Complete, PASS | Exact merge `d472c6bd3` has parents `07929bf6a` and `6e52b8f85`, changes exactly `.gitignore` plus `tools/tests/test_disc_ignore.py`, passes the deterministic five-test fixture, and reports zero tracked ISO/GCM/CISO paths. This is ignore-policy proof only; it does not access or prove asset contents. |
| 414 | Nested ROM umbrella integration | Complete and integrated | Independent immutable review passed and [umbrella PR #31](https://github.com/jskoiz/acgc-macos/pull/31) merged as `94cdee0b3`, advancing the PC gitlink and runner pin to `d472c6bd3` with synchronized README/lane-board/evidence. The runtime/full-link frontier remains exact `9860ebc5c`; no renderer or playability claim follows. |
| 415 | Exact-PID host-runner cleanup replay | Candidate; local commit pending immutable review | Umbrella-only replay on fresh branch `c1/lane-exact-pid-cleanup-714` from live `origin/main` `714958f46`, carrying the reviewed `cfaf226ca` two-script ownership patch plus synchronized README, lane-board, and [dedicated evidence](evidence/EXACT-PID-CLEANUP-714958F46-2026-08-24.md). Bounded bash/zsh syntax, help, and fake-callback gates are source-contract proof only; no real launch, signal, LLDB, workflow, PC/decomp, or asset claim follows. Useful successor: independent immutable review, then the integration owner’s live-base/auth/workflow checks and umbrella PR/merge. |
...
20. ~~Determine whether the current eight-map Texture state, two-stage TEV use,
   ...
   exactly one source owner only after the read-only audits converge~~ — done;
   the non-V2 synchronous resource-handoff owner merged through PC PR #35 as
   `586cf7a61`, with borrow-scoped CPU staging, atomic callback ownership, and
   fresh exact-merge PC/Apple native plus combined ASan/UBSan proof. The live
   status-17/no-sink frontier remains unrerun.
21. Replay the reviewed download-manifest and nested-ROM hygiene changes one at
   a time without displacing or overlapping the atomic resource owner; the
   nested-ROM replay is integrated at `d472c6bd3`, and the exact-current
   download-integrity replay has a descriptor-anchored corrective candidate
   under independent immutable review.
